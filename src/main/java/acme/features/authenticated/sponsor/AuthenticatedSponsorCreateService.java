...
public class AuthenticatedSponsorCreateService extends AbstractService<Authenticated, Sponsor> {
	...
	public void validate(final Sponsor object) {
		assert object != null;
		if (!super.getBuffer().getErrors().hasErrors("id")) {
			Sponsor existing;

			existing = this.repository.findOneSponsorById(object.getId());
			super.state(existing == null, "code", "sponsor.form.error.duplicated");
		}
	}
